...
一個簡單的手寫數字識別系統，使用卷積神經網路（CNN）和 MNIST 數據集。包含模型訓練、預測以及一個 Flask Web 應用來與模型互動。
...
📢 **2️⃣** 執行 `train.py` 來載入 MNIST 數據集、訓練模型並儲存。

📢 **3️⃣** 執行 `app.py` 來啟動 Flask 伺服器。可以上傳手寫數字圖片，並進行判別是什麼數字。
