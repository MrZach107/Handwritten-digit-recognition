...
**1️、** 執行 `mnist_cnn.py`　來生成 PyTorch 模型（`mnist_cnn.pth`）。

**2️、** 執行 `train.py` 以載入 MNIST 數據集、訓練模型並儲存。

**3️、** 執行 `app.py` 以啟動 Flask 伺服器。可以上傳手寫數字圖片，並進行判別是什麼數字。
